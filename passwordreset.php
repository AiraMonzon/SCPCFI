...
if(isset($_POST['reset_password'])){
        ...
        if(empty($new_password)){
                header("Location: passwordreset.php?error=New Password is required");
                exit();
        }
        else if(!preg_match("/^[a-zA-Z0-9\w\S]+$/", $new_password))
        {
                header("Location: passwordreset.php?error=Password must contain letters, numbers or symbols only&$user_data");
                exit();
        }
        else if(strlen($new_password) <8)
        {
                header("Location: passwordreset.php?error=Password is too short must be 8-32 characters length&$user_data");
                exit();
        }
        else if(strlen($new_password) >32){
                header("Location: passwordreset.php?error=Password is too long must be 8-32 characters length&$user_data");
                exit();
        }
        else if(empty($verify_password)){
                header("Location: passwordreset.php?error=Confirm Password is required");
                exit();
        }                
        else if($new_password !== $verify_password){
                header("Location: passwordreset.php?error=The confirmation password does not match");
                exit();
        }
        else
        {
...
$sql2= "SELECT * FROM code WHERE verification_code= '$random_code'";
                ...
                if (mysqli_num_rows($result) && mysqli_num_rows($result2) > 0)  
                {
                        ...
                        if($result2)
                        {
                                ...
                                header("Location: passwordreset.php?successsend=Password was reset successfully");
                                exit();
                        }
                        else
                        {
                                header("Location: passwordreset.php?error=Unknown error occurred");
                                exit();
                        }
                }
                else
                {
                        header("Location: passwordreset.php?error=The email/verification code is not existing in this site");
                        exit();
                }
